...
export class TwistedBow extends RangedWeapon {
  ...
  _accuracyMultiplier(from: Unit, to: Unit, bonuses: AttackBonuses) {
    const magic = Math.min(
      Math.max(to.currentStats.magic, to.bonuses.attack.magic),
      250
    );
    const multiplier =
      (140 +
        ((10 * 3 * magic) / 10 - 10) / 100 -
        Math.pow((3 * magic) / 10 - 100, 2) / 100) /
      100;
  ...
  _damageMultiplier(from: Unit, to: Unit, bonuses: AttackBonuses) {
    ...
    const multiplier =
      (250 +
        ((10 * 3 * magic) / 10 - 14) / 100 -
        Math.pow((3 * magic) / 10 - 140, 2) / 100) /
      100;
